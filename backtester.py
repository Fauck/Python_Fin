...
def _cached_backtest(
    ...
    return run_backtest(df, strategy_fn, take_profit_pct, stop_loss_pct, exit_on_ma20)
...
def render_backtest_page() -> None:
    ...
    with ctrl_col:
        ...
        stop_loss_pct = st.slider(
            "停損（%）", min_value=1, max_value=20, value=5, step=1,
            key="bt_sl",
            help="收盤跌至買入價 × (1 - 停損%) 時出場",
        ) / 100.0
